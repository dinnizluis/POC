...
u2r_n_normal = ['buffer_overflow', 'loadmodule', 'rootkit', 'perl', 'sqlattack', 'xterm', 'ps', 'normal']
ind = []
for index, row in df.iterrows():
	if(df['label'][index] not in u2r_n_normal):
		ind.append(index) 
...
df_7 = df_7[['Num_file_creations', 'Num_shells']]
...
df_7.to_csv(path_in + 'nslkdd_u2r.csv', sep='\t', index = False)
