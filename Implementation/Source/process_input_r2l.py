...
r2l_n_normal = ['guess_passwd', 'ftp_write', 'imap', 'phf', 'multihop', 'warezmaster', 'warezclient', 'spy', 'xlock', 'xsnoop', 'snmpguess', 'snmpgetattack', 'httptunnel', 'sendmail', 'named', 'normal']
ind = []
for index, row in df.iterrows():
	if(df['label'][index] not in r2l_n_normal):
		ind.append(index) 
...
df_7 = df_7[['Duration', 'Num_failed_logins']]
...
df_7.to_csv(path_in + 'nslkdd_r2l.csv', sep='\t', index = False)
